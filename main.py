...
# Part (a): Construct simple linear regression models (Models 1–6)
for i, (X_var, var_name) in enumerate(zip([X1, X2, X3, X4, X5, X6], variables)):
    ...
    # Fit statsmodels for p-values
    X_var_sm = sm.add_constant(X_var)  # Add intercept
    ...
    p_value = model_sm.pvalues[1]  # p-value for the slope
    ...
    print(f"Hypothesis: {'Reject H₀' if p_value < 0.05 else 'Fail to reject H₀'} (H₀: β_{var_name} = 0)")
...
# Part (a): Construct multiple linear regression model (Model 7)
# Fit sklearn model
...
p_values_multi = model_multi_sm.pvalues[1:]  # Exclude intercept
...
for var, p_val in zip(variables, p_values_multi):
    ...
    print(f"Hypothesis: {'Reject H₀' if p_val < 0.05 else 'Fail to reject H₀'} (H₀: β_{var} = 0)")
...
print(f"Reason: The model has the highest Adjusted R² ({best_model['Adjusted R²']:.4f}), "
      f"indicating it explains the most variance in Y while accounting for the number of predictors.")
print("Additional Insights:")
print("- Simple linear models with low R² (e.g., X2, X6) suggest weak linear relationships.")
print("- The multiple regression model may overfit if many predictors are insignificant (check p-values).")
print("- MSE indicates prediction error; lower MSE is better, but Adjusted R² is prioritized for model selection.")